  ...
  return (
    <div className="bg-white border p-6 mt-4 rounded-lg shadow-md dark:bg-gray-800 dark:text-white dark:border-black dark:border-3 ">
      ...
      <div className="flex gap-4 mt-4">
        {story.state !== "doing" && (
          <button onClick={() => handleStatusChange("doing")} className="bg-yellow-400 text-black px-4 py-2 rounded hover:bg-yellow-500">
            Oznacz jako "doing"
          </button>
        )}
        {story.state !== "done" && (
          <button onClick={() => handleStatusChange("done")} className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
            Oznacz jako "done"
          </button>
        )}
